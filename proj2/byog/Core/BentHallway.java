...
public class BentHallway extends Hallway {
    ...
    public BentHallway(Position p1, Position p2, Position p3, TETile[][] world, TETile floor,
        TETile wall) {
        if (p1 == null || p2 == null || p3 == null || world == null || floor == null
            || wall == null) {
            throw new IllegalArgumentException("Tried to initialize " + getClass()
                + " with null argument(s).");
        }
        if (!(p1.alignedOnX(p2) && p2.alignedOnY(p3)) && !(p1.alignedOnY(p2)
            && p2.alignedOnX(p3))) {
            throw new IllegalArgumentException("Tried to initialize " + getClass()
